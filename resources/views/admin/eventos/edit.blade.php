    ...
    <form action="{{ route('admin.eventos.update', $evento->id) }}" method="POST" class="space-y-6">
      ...
      <div>
        ...
        <textarea id="descricao" name="descricao" rows="3" required
                  class="w-full px-5 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0A2D35] transition">{{ old('descricao', $evento->descricao) }}</textarea>
        @error('descricao')
          <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div>
        ...
        <input id="data_evento" type="datetime-local" name="data_evento" value="{{ old('data_evento', \Carbon\Carbon::parse($evento->data_evento)->format('Y-m-d\TH:i')) }}" required
               class="w-full px-5 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0A2D35] transition" />
        @error('data_evento')
          <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
